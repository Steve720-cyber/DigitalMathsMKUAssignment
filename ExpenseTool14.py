...
def open_meal_window():
    ...
    date = date_var.get()
    fg1='black'
#font_valid = ('Times', 16, 'normal', color='green')
 #   font_10 = ('Times', 16, 'normal', color='amber')
  #  font_over = ('Times', 16, 'normal', color='red')
    ...
    total_entry = Entry(meal_window, textvariable=total_var, font=('calibre', 10, 'normal'), disabledforeground= fg1, state=DISABLED)
    ...
    def confirm_entries():
        ...
        if (lunch.isdigit() and dinner.isdigit() and other.isdigit()):
            ...
            tkinter.messagebox.showwarning("Invalid", "Invalid Data, Expenses Must Be Numeric Please")

    #----------------------------------------Buttons-------------------------------------------------------------------

    confirm_button = (Button(meal_window, text="Confirm Entries", bg = "#5f9ea0",  command=confirm_entries).grid(row=10, column=1, columnspan=2, sticky="nsew"))
