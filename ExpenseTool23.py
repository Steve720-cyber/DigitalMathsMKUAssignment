...
class BudgetApp:
    def __init__(self, root):
        self.root = meal_window
        self.root.title("Meal Budget")
...
#     meal_window.wm_transient(master)
    ...
    self.lunch_var = StringVar()
    self.dinner_var = StringVar()
    ...
    self.budget_label = Label(meal_window, text = "Preset Daily Budget: ")
    self.date_label = Label(meal_window, text="Date (DD/MM/YY):")
    self.lunch_label = Label(meal_window, text="Input Lunch Expenses: ")
    self.dinner_label = Label(meal_window, text="Input Dinner Expenses: ")
    self.other_label = Label(meal_window, text="Input Other Expenses: ")
    self.total_label = Label(meal_window, text="Total Daily Expenses: ")
    self.under_label = Label(meal_window, text="Under or equal to budget", bg='green')
    self.over_label = Label(meal_window, text="Over budget, but within 10%", bg='#FFBF00')
    self.excess_label = Label(meal_window, text="Exceeds budget parameters", bg='red')
    self.experiment_label = Label(meal_window, text="")

    #-------------------------------Entries------------------------------------------------------------------------

    self.budget_entry = Entry(meal_window, textvariable=budget_var, font=('calibre', 10, 'normal'))
    self.budget_entry.insert(0, "40")

    self.date_entry = tkc.DateEntry(meal_window, date_pattern='dd/mm/yyyy', font=('calibre', 10, 'normal'))

    self.lunch_entry = Entry(meal_window, textvariable=lunch_var, font=('calibre', 10, 'normal'))
    #lunch = lunch_entry.get()
    #lunch_entry_float = float(lunch)

    self.dinner_entry = Entry(meal_window, textvariable=dinner_var, font=('calibre', 10, 'normal'))
    #dinner = dinner_entry.get()
    #dinner_entry_float = float(dinner_entry)

    self.other_entry = Entry(meal_window, textvariable=other_var, font=('calibre', 10, 'normal'))
    #other = other_entry.get()
    #other_entry_float = float(other_entry)

    self.total_entry = Entry(meal_window, textvariable=total_var, font=('calibre', 10, 'normal'))
...
#===============================================================================================================
    ...
    def view_entries(self):
        global df
        self.top = Toplevel(self.meal_window)
        ...
        for index, row in df.iterrows():
            ...
            edit_button = Button(self.top, text="Edit", command=lambda i=index: self.edit_entry(i))
            delete_button = Button(self.top, text="Delete", command=lambda i=index: self.delete_entry(i))
    ...
    def edit_entry(self, index):
        global df
        self.edit_top = Toplevel(root)
        self.edit_top.title("Edit Entry")

        self.edit_budget = Entry(edit_top)
        self.edit_budget.insert(0, df.at[index, 'Budget'])
        self.edit_budget.pack()

        self.edit_date = Entry(edit_top)
        self.edit_date.insert(0, df.at[index, 'Date'])
        self.edit_date.pack()

        self.edit_lunch = Entry(edit_top)
        self.edit_lunch.insert(0, df.at[index, 'Lunch'])
        self.edit_lunch.pack()

        self.edit_dinner = Entry(edit_top)
        self.edit_dinner.insert(0, df.at[index, 'Dinner'])
        self.edit_dinner.pack()

        self.edit_other = Entry(edit_top)
        self.edit_other.insert(0, df.at[index, 'Other'])
        self.edit_other.pack()

        self.save_button = Button(edit_top, text="Save", command= lambda i=index: save_edit(i))
        self.save_button.pack()


    def save_edit(self, index):
        global df
        df.at[index, 'Budget'] = self.edit_budget.get()
        df.at[index, 'Date'] = self.edit_date.get()
        df.at[index, 'Lunch'] = float(self.edit_lunch.get())
        df.at[index, 'Dinner'] = self.edit_dinner.get()
        df.at[index, 'Other'] = self.edit_other.get()
        df.to_csv('./records.csv',index=False)
