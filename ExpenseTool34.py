...
# Check if records.csv file exists already, if not create new dataframe. This will only happen
# the first time the app is run, or if the csv file is deleted or moved.
...
master = Tk()
master.geometry("480x150+100+100")  # Set window size
...
def open_meal_window():
    ...
    x = master.winfo_x()
    y = master.winfo_y()
    meal_window.geometry("+%d+%d" % (x + 300, y + 50))
    ...
    def confirm_entries():
        ...
        # Following snippet attempts to make the entries floats, this will deal with integers and floats,
        # and also identify blanks or alpha characters. In the case of the last two types, the message
        # explains, and the return False will hold the window for correct entries.
        ...
        try:
            ...
            messagebox.showerror("Error", "All fields are required and must be numeric.")
            return False

        else:
            entries = float(lunch) + float(dinner) + float(other)

            # Next line restricts total field to 2 decimal places. Python floats sometimes give surprising
            # results such as 1.1 + 1.1 + 1.1 = 3.3000000000000003

            entries2 = ("%.2f" % entries)
            total_var.set(entries2)
        ...
        dinner = float(dinner)
        other = float(other)
        if (isinstance(lunch, float) and isinstance(dinner, float) and isinstance(other, float)):
            entries = float(lunch) + float(dinner) + float(other)

            if entries <= float(budget):
                boundary_label['text'] = "Under or equal to budget"
                boundary_label['bg'] = 'Green'
            elif entries <= float(budget2):
                boundary_label['text'] = "Over budget, but within 10%"
                boundary_label['bg'] = 'Yellow'
            else:
                boundary_label['text'] = "Exceeds budget parameters"
                boundary_label['bg'] = 'Red'
...
#===============================================================================================================
    ...
    def view_entries():
        ...
        # ----------------------------------Scrollbar---------------------------------------
        v = Scrollbar(top, orient='vertical', width=30)
        ...
        def edit_entry(index):
            ...
            edit_top.geometry("300x150+300+200")
            edit_top.config(bg='lightblue')
            ...
            def save_edit(index):
                ...
                df.at[index, 'Budget'] = float(edit_budget.get())
                ...
                # newLunch = ("%.2f" % newLunch)
                # newDinner = ("%.2f" % newDinner)
                # newOther = ("%.2f" % newOther)
...
#--------------------------------------------------------------------------------
    ...
    boundary_label.grid(row=11, column=6, columnspan=2)
...
#label_today.pack()
#label_date.pack(
label_today.grid(row=5, column=4, padx=2)
label_date.grid(row=5, column=6, padx=2)

#button1.grid(row=10, column=6, rowspan=2, columnspan = 6)
#button2.grid(row=7, column=1, columnspan=2, sticky="nsew")
#button3.grid(row=15, column=6, rowspan = 2, columnspan=6)

button1.grid(row=10, column=5, columnspan=1, pady = 10)
#button2.grid(row=7, column=1, columnspan=2, sticky="nsew")
button3.grid(row=15, column=5, columnspan=1, pady = 10)
